...
export const SubscriptionSubject = ({
	subscriptionList,
	loadRazorpay,
	handleModeChange,
	subjectMode,
	...
	return (
		...
		>
			<Grid item xs={12}>
				<Typography
					variant="body2"
					sx={{ fontSize: "16px", fontWeight: 500, fontFamily: "Urbanist" }}
				>
					<Box mb={2} sx={{ textAlign: "center" }}>
						You are Currently on{" "}
						{subscriptionList ? subscriptionList.subscribedCourses.length : 0}{" "}
						Subject
						{subscriptionList ? (subscriptionList.length > 1 ? "s" : "") : ""}{" "}
						Subscription, upgrade license to avail more benefits!
					</Box>
				</Typography>
			</Grid>
			<Grid item xs={12}>
				{subscriptionList?.subscribedCourses.length < 3 && (
					<Box onClick={() => handleModeChange(1)}>
						<Grid
							container
							style={subjectMode === 1 ? selectedStyle : unSelectedStyle}
						>
							<Grid
								item
								xs={6}
								...
								<Typography variant="body2">
									<Box
										...
										sx={{
											...
											fontFamily: "Urbanist",
											fontWeight: "400",
											display: { xs: "center", sm: "center", lg: "left" },
										}}
									>
										{subscriptionList?.subscribedCourses.length > 0
											? "₹250"
											: "₹300"}{" "}
										for Year
									</Box>
											...
											subjectList.map((subject) => {
												return (
													<Chip
														...
														label={subject.name}
														sx={
															subscriptionList?.subscribedCourses.includes(
																subject.id
															)
																? disabledChip
																: subjectMode === 1 &&
			...
			<Grid item xs={12}>
				{subscriptionList?.subscribedCourses.length < 2 && (
					<Box onClick={() => handleModeChange(2)}>
						<Grid
							container
							style={subjectMode === 2 ? selectedStyle : unSelectedStyle}
						>
							<Grid
								item
								xs={6}
			...
			<Grid item xs={12}>
				{subscriptionList?.subscribedCourses.length < 1 && (
					<Box onClick={() => handleModeChange(3)}>
						<Grid
							container
							style={subjectMode === 3 ? selectedStyle : unSelectedStyle}
						>
							<Grid
								item
								xs={6}
