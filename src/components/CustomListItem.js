    ...
    return <Box>
        ...
        <Grid container style={{ backgroundColor: '#F7F8F9', borderRadius: '0px 20px 20px 20px', padding: '20px', marginBottom: '20px' }}>
            ...
            <Grid item xs={12} sm={6} md={4} lg={4} style={{ justifyContent: 'flex-start', alignItems: 'center' }}>
                <Box variant='body2' mt={1}>
                    <Box sx={{ fontFamily: 'Urbantist', fontWeight: 500, color: 'rgba(106,112,124,1)' }}> Test Length: {props.option.questions.length} Questions</Box>
                </Box>
                <Box variant='body2' mt={1}>
                    <Box sx={{ fontFamily: 'Urbantist', fontWeight: 500, color: 'rgba(106,112,124,1)' }}>Test Time: {props.option.totalDurationMin} Minutes</Box>
                </Box>
        </Grid>
        <Grid item xs={12} sm={6} md={8} lg={8} style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
            ...
            {props.option.testStatus == null
                ? <CustomButton onClick={() => { props.testScreen(props.option._id) }} btnText="ENTER" endIcon={<PlayArrowOutlinedIcon />} variant="contained" sx={{ color: "white", borderRadius: "20px", width: '105px', height: { xs: "34px", lg: '36px' }, fontSize: { xs: "8px", lg: '12px' }, backgroundColor: "gba(131, 139, 161, 1)", fontWeight: 600 }} />
