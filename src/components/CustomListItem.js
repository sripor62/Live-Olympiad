...
export const CustomListItem = (props) => {
    ...
    return <Box>
        ...
        <Grid container item xs={12} sm={12} md={12} lg={12} style={{ backgroundColor: '#F7F8F9', borderRadius: '0px 20px 20px 20px', padding: '20px', marginBottom: '20px' }}>
            ...
            <Typography sx={{ fontSize: "18px", display: 'flex'}} ><Box sx={{ fontSize: '50px', color: 'green' }}></Box><Box sx={{ mt: 4 ,mr:4}}>/{props.option.totalMarks}</Box></Typography>
            <CustomButton onClick={()=>{props.testScreen(props.option._id)}} btnText="RE-TAKE" endIcon={<RefreshIcon/>} variant="contained"  sx={{width:'105px',height:{xs:"34px",lg:'36px'},fontSize:{xs:"8px",lg:'12px'},backgroundColor:"#838BA1" ,fontWeight:600}}/>
