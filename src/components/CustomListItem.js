    ...
    return <Box>
        ...
        <Grid container item xs={12} sm={12} md={12} lg={12} style={{ backgroundColor: '#F7F8F9', borderRadius: '0px 20px 20px 20px', padding: '20px', marginBottom: '20px' }}>
            ...
            <Grid item xs={8} sm={6} md={6} lg={8} style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
                ...
                {!!props.option.attemptedQuestions && <Typography sx={{ fontSize: "18px", display: 'flex' }} ><Box sx={{ fontSize: '50px', color: 'green' }}>{props.option.attemptedQuestions ? props.option.attemptedQuestions : 0}</Box><Box sx={{ mt: 4, mr: 4 }}>/{props.option.totalMarks}</Box></Typography>}
